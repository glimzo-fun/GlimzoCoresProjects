...
public class PunishmentManager {
    ...
    // UUID -> list of punishments loaded on join
    ...
    public void loadPunishmentsFromData(UUID uuid, java.util.List<java.util.Map<String,Object>> rows) {
        java.util.List<Punishment> list = new CopyOnWriteArrayList<>();
        for (java.util.Map<String,Object> row : rows) {
            ...
            try {
                list.add(new Punishment(
                        ...
                        staffStr != null ? java.util.UUID.fromString(staffStr) : null,
                        (String) row.get("staff_name"),
                        me.pikashrey.glimzocore.api.punishment.PunishmentType.valueOf((String) row.get("type")),
                        (String) row.get("reason"),
                        (long) row.get("issued_at"),
                        (long) row.get("expires_at"),
                        (boolean) row.get("active"),
                        (boolean) row.get("appealed")
                ));
    ...
    public boolean punish(Punishment punishment) {
        ...
        // Update PlayerData flags for ban/mute
        PlayerData data = GlobalPlayer.get(punishment.getTargetUuid());
        if (data != null) {
            syncToPlayerData(data, punishment);
        }

        Bukkit.getScheduler().runTaskAsynchronously(plugin, () -> {
            saveToDb(punishment);
            // BanCheckListener reads banned/ban_expiry/ban_reason directly from
            // glimzo_players at login. syncToPlayerData() only updates the in-memory
            // PlayerData - for offline players that object doesn't exist, so we must
            // also stamp the columns on glimzo_players directly.
            // We do this unconditionally (online or offline) - for online players the
            // autosave/quit-save would eventually do it, but writing it now is safer
            // and prevents a window where a quick-reconnect bypasses the ban.
    ...
    public void unban(UUID targetUuid, UUID staffUuid, String staffName) {
        ...
        PlayerUnPunishEvent event = buildUnpunishEvent(targetUuid, staffUuid, staffName, false);
        Bukkit.getPluginManager().callEvent(event);

        Bukkit.getScheduler().runTaskAsynchronously(plugin, () -> {
            // Always deactivate punishment rows
            deactivateInDb(targetUuid, PunishmentType.BAN, PunishmentType.TEMP_BAN);
            // Also clear the ban flag on glimzo_players - this is what BanCheckListener
            // reads on login, so it MUST be updated even for offline/permanent bans.
    ...
    public void unmute(UUID targetUuid, UUID staffUuid, String staffName) {
        ...
        PlayerUnPunishEvent event = buildUnpunishEvent(targetUuid, staffUuid, staffName, true);
        Bukkit.getPluginManager().callEvent(event);
    ...
    public boolean isBanned(UUID uuid) {
        ...
        // Offline check from cache or DB
        List<Punishment> punishments = cache.getOrDefault(uuid, Collections.emptyList());
    ...
    public Punishment getActiveBan(UUID uuid) {
        return getActiveByType(uuid, PunishmentType.BAN, PunishmentType.TEMP_BAN);
    }

    public Punishment getActiveMute(UUID uuid) {
        return getActiveByType(uuid, PunishmentType.MUTE, PunishmentType.TEMP_MUTE);
    }

    public List<Punishment> getHistory(UUID uuid) {
        return Collections.unmodifiableList(
                cache.getOrDefault(uuid, Collections.emptyList()));
    }

    // Ban screen

    public String buildBanScreen(Punishment p) {
        me.pikashrey.glimzocore.managers.ConfigManager cfg =
                me.pikashrey.glimzocore.GlimzoCore.getInstance().getConfigManager();
        ...
        String expiry    = p.isPermanent() ? permFmt
                : expFmt.replace("{duration}", formatExpiry(p.getExpiresAt()));
    ...
    public String buildMuteMessage(Punishment p) {
        ...
        String expiry  = p.isPermanent() ? permFmt
                : expFmt.replace("{expires}", formatExpiry(p.getExpiresAt()));
        return CC.translate(header + "\n" + reason + "\n" + expiry);
    }
    ...
    private Punishment getActiveByType(UUID uuid, PunishmentType... types) {
        Set<PunishmentType> typeSet = new HashSet<>(Arrays.asList(types));
        List<Punishment> list = cache.getOrDefault(uuid, Collections.emptyList());
        for (Punishment p : list) {
    ...
    private PlayerUnPunishEvent buildUnpunishEvent(UUID target, UUID staff, String staffName, boolean mute) {
        me.pikashrey.glimzocore.api.punishment.Punishment apiPunishment =
                new me.pikashrey.glimzocore.api.punishment.Punishment(
                        -1, target, "?", staff, staffName,
                        mute ? PunishmentType.MUTE : PunishmentType.BAN,
                        "revoked", System.currentTimeMillis(), -1, false, false);
        return new PlayerUnPunishEvent(apiPunishment);
    }
    ...
    private void saveToDb(Punishment p) {
        String sql =
                "INSERT INTO glimzo_punishments " +
