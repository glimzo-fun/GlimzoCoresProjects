...
public class BanCheckListener implements Listener {
    ...
    public void onPreLogin(AsyncPlayerPreLoginEvent event) {
        ...
        if (vpnResult == VpnChecker.Result.VPN_OR_PROXY) {
            if (!hasBaronOrAbove(uuid)) {
                event.disallow(AsyncPlayerPreLoginEvent.Result.KICK_OTHER, buildVpnKickMessage());
            }
        }
    }
    ...
    private String buildBanMessage(String reason, long expiry) {
        ...
        if (expiry == -1) {
            ...
            String formatted = remaining > 0 ? ChatListener.formatRemaining(remaining) : "Expired";
    ...
    private boolean hasBaronOrAbove(UUID uuid) {
        ...
        if (bypassWeight <= 0) return false;
        ...
        try {
            ...
            if (loader != null) {
                for (me.pikashrey.glimzocore.features.rank.ConfiguredRank rank : loader.getAll()) {
                    if (rank.getWeight() >= bypassWeight) qualifyingIds.add(rank.getId().toLowerCase());
                }
            }
        } catch (Exception e) {
            ...
            qualifyingIds.addAll(java.util.Arrays.asList(
                    ...
                    "guide", "guardian", "pibble", "architect", "admin", "chief"
            ));
        }
        ...
        for (int i = 0; i < qualifyingIds.size(); i++) inClause.append(i == 0 ? "?" : ",?");
        ...
        try (Connection con = plugin.getMysqlManager().getConnection();
            ...
            int idx = 3;
            for (String id : qualifyingIds) ps.setString(idx++, id);
            try (ResultSet rs = ps.executeQuery()) {
                return rs.next();
            }
        } catch (Exception e) {
            plugin.log("&c[VPN] Rank query failed for " + uuid + ": " + e.getMessage() + " - failing open.");
            return true;
        }
    }
