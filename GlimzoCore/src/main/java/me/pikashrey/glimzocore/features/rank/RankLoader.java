...
public class RankLoader {
    ...
    public void load() {
        ...
        for (String id : root.getKeys(false)) {
            ...
            ConfiguredRank rank = new ConfiguredRank(
                    id,
                    rs.getString("prefix", "&7"),
                    rs.getInt("weight", 0),
                    rs.getStringList("permissions"),
                    rs.getStringList("inheritance"),
                    rs.getBoolean("staff", false),
                    rs.getBoolean("donor", false),
                    rs.getBoolean("default", false),
                    rs.getInt("max-clan-size", 0)
            );
        ...
        for (String id : ranks.keySet()) {
            cache.cacheRank(id, resolver.resolve(id, inheritance, permissions));
        }
    ...
    private Map<String, List<String>> buildInheritanceMap() {
        ...
        for (ConfiguredRank r : ranks.values()) map.put(r.getId(), new ArrayList<>(r.getInheritance()));
        return map;
    }

    private Map<String, List<String>> buildPermissionsMap() {
        Map<String, List<String>> map = new HashMap<>();
        for (ConfiguredRank r : ranks.values()) map.put(r.getId(), new ArrayList<>(r.getPermissions()));
        return map;
    }
        ...
        for (Map.Entry<String, List<String>> e : inheritance.entrySet()) {
            for (String parent : e.getValue()) {
                if (!ranks.containsKey(parent.toLowerCase()))
                    throw new IllegalStateException("Rank " + e.getKey() + " inherits unknown parent '" + parent + "'");
            }
        }
    }
    ...
    private void validateNoLoops(Map<String, List<String>> inheritance) {
        ...
        for (String id : inheritance.keySet()) dfs(id, inheritance, visiting, visited);
    }

    private void dfs(String id, Map<String, List<String>> inheritance,
                     Set<String> visiting, Set<String> visited) {
        if (visited.contains(id)) return;
        if (visiting.contains(id)) throw new IllegalStateException("Inheritance loop at rank: " + id);
        visiting.add(id);
    ...
    public ConfiguredRank get(String id) { return id == null ? null : ranks.get(id.toLowerCase()); }
    public Collection<ConfiguredRank> getAll() { return Collections.unmodifiableCollection(ranks.values()); }
}
